...
import CustomSelectEditor from 'components/CustomSelectEditor';
...
//      hasPermission,
  ...
  const myTheme = themeQuartz.withParams({
    ...
    headerBackgroundColor: "black", /*"#f2f3f7",*/
...
//      singleClickEdit: true,
    },
    {
      ...
      cellEditorParams: {
        options: gridUseYnOptions,
      },
      valueFormatter: (params) => getLabelFromId(params.value), // id → name 표시
      valueParser: (name) => getIdFromLabel(name), // name → id 저장
//      singleClickEdit: true,
    },
  ]);
  ...
  const getLabelFromId = (id) => {
    const match = gridUseYnOptions.find((item) => item.id === id);
    return match ? match.name : id;
  };
  // 그리드 콤보 name -> id
  const getIdFromLabel = (name) => {
    const match = gridUseYnOptions.find((item) => item.name === name);
