...
const WhLocMgmt = () => {
  ...
  // 그리드 테마
  const { agGridTheme } = useContext(GlobalContext);
...
//      hasPermission,
  ...
  // 공통코드 상태
  const [gridUseYn] = useState([]);         // 그리드에서 사용할 사용여부 콤보 옵션
...
//      console.log('공통코드 조회 결과 :::', resData);
      ...
      (resData.find(item => item.USE_YN_N)?.USE_YN_N || []).forEach((result)=>{
        ...
        if ( !gridUseYn.find(item => item.id === result.SML_CD) ) {
            gridUseYn.push(temp);
        }
      });
//      console.log('gridUseYn  :::  ', gridUseYn.map((opt) => opt.name));
...
//      console.log('창고위치코드 조회 결과 :::', resData);
  ...
  }, [whLocSearch, userInfo, validateFields, validationRules.whLocSearch]);
      ...
      <div className="code-management-container">
        ...
        <div className="left-section-fullWidth">
          <div className="paper" style={{ height: 'calc(100% - 12px)' }}>
            ...
            {/* 중간 - 데이터 영역 */}
            <div className="data-section ag-theme-alpine" style={{ height: '550px' }}>
              <AgGridReact
                ref={gridRef}
                theme={agGridTheme}
                rowData={whLocCodes}
...
*/
                  ...
                  },
                  {
                    ...
                    cellEditorParams: {
                      values: gridUseYn.map((opt) => opt.name), // 셀렉트 박스에 표시할 label 목록
                    },
                    valueFormatter: (params) => getLabelFromId(gridUseYn, params.value), // id → name 표시
                    valueParser: (name) => getIdFromLabel(gridUseYn, name), // name → id 저장
                    options: [...gridUseYn],    // 사용자정의로 셋팅
                    /*editable: true,*/
                    /*singleClickEdit: true,*/
                  },
                ]}
